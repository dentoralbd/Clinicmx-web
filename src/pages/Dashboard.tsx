...
import { fetchDashboardData } from '@/repositories/dashboardRepo'
...
export function Dashboard() {
  ...
  async function loadDashboardData(_isRefresh = false) {
    await refetch()
  }
...
function BackupHealthTile({ onClick }: { onClick: () => void }) {
  ...
  useEffect(() => {
    ...
    getDriveBackupStatus()
      ...
      .catch(() => {
        if (!cancelled) setDriveError(true)
      })
    return () => {
      cancelled = true
    }
          ...
          <span className="text-sm">
            ...
            {driveError
              ...
              : !drive
                ? 'checking…'
                : lastBackupAt
                  ? formatDistanceToNow(lastBackupAt, { addSuffix: true })
                  : 'never'}
          </span>
        </div>
