...
| `text-primary` / `text-secondary` | `#1B2733` / `#5A7184` | Body text / muted text |
...
- Font: **Inter**, falling back to system-ui (`index.css` body rule). No font files are bundled — system fallback in practice.
...
- Shadows: `shadow-elevation-low` (cards at rest) → `elevation-md` (hover/dropdowns) → `elevation-high` (modals, FAB). Defined in the Tailwind config; don't invent ad-hoc shadows.
...
- Shared PDFs (Email/WhatsApp) are generated by capturing the same print components at **forced desktop width** (`domToPdf.ts`) so a phone-shared PDF is identical to the printed page. Any print-layout change must be checked in three modes: browser print, downloaded PDF, WhatsApp-shared PDF.
