...
- **M3 auth shape:** all roles become Supabase Auth users; staff get synthetic emails from their identifier; permissions stay in `app_users`, joined in RLS via SECURITY DEFINER helpers (no custom JWT claims). Admin PIN `6040` survives as the offline unlock / encryption-key gate layered over a persisted Supabase session.
...
- Auth: admin PIN `6040` compared client-side → **admin login works offline by construction**. Doctor/operator login queries `app_users` → needs network until M3. The sessionStorage AES key dies when the tab closes, so an offline cold start always lands on Login; the admin PIN path works there.
