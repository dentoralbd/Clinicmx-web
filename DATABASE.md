...
| `dental_records` | Per-tooth chart records (tooth number, condition, notes). One row per `(patient_id, tooth_number)` — latest state only. `updated_at` trigger. |
| `dental_record_history` | **068 —** append-only dated log powering the Anatomic Odontogram's Timeline & History Scrubber: `patient_id`, `tooth_number`, `condition`, `notes`, `procedure_date`, `doctor_name`, `created_at`. Never updated in place; `saveToothCondition()` (PatientProfile) inserts one row per save alongside the `dental_records` upsert. Migration backfilled one snapshot per existing `dental_records` row. Chart replays these (≤ target date, last-write-per-tooth-wins) via `computeChartSnapshot()` in `src/components/dental/AnatomicDentalChart.tsx`. Permissive "Allow all" RLS mirroring `dental_records`. |
...
| `treatment_catalog_items` | Clinic-added procedures: `category_id` FK, `name` (unique), `default_fee` (nullable), `default_duration_mins` (nullable, added by 061 — chair-time used by the Patient Queue's ETA engine, editable on `/catalog`), `chart_condition` (nullable, **069** — the tooth-chart condition this procedure auto-applies when its treatment goes In Progress/Completed; a `ToothCondition` code, the `'__none__'` sentinel = suppress, or NULL = keyword-guess; drives `src/lib/toothChartSync.ts`, editable on `/catalog`). Backs the shared `<TreatmentTypeSelect>` component used by all 4 `treatment_type` dropdown sites (`Treatments.tsx` x2, `PatientProfile.tsx` x2). |
