...
- **Windows desktop app (2026-08-09, Tauri v2, `src-tauri/`):** does *not* auto-update from a deploy like the APK — it bundles its own copy of `dist/` at build time and needs a fresh `npm run tauri build` + reinstall to pick up changes. See CLAUDE.md § Windows desktop app.
