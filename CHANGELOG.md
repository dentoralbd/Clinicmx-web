...
- **Account-scoped approval:** a shared device can accumulate offline edits from more than one login; syncing/discarding a queued edit is restricted to the account that created it (`canActOn()` in `offlineSync.ts`), same trust model as `canDelete()`/`canRevert()`. **Admin is exempt** — can act on any account's queued edits, from both the existing `/offline-outbox` entry and a new **Admin zone → Offline Edits** tile (`DoctorProfile.tsx`) that shows every pending edit on the device with who made it.
