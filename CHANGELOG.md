...
## 2026-08-09 — Windows desktop app (Tauri v2)
Anti-Gravity had prototyped a Tauri v2 desktop wrapper in a separate fork (`Clinicmx-web-redesign`) for exploration purposes. Rebuilt properly here, in this app, so the desktop build ships the actual production feature set instead of a diverging UI experiment.

- Scaffolded `src-tauri/` (Tauri v2, WebView2, window/icon config, `dialog`/`fs`/`opener` plugins). Regenerated `src-tauri/icons/` from the real `public/logo.png` mark (the redesign fork's copy had been generated from the unrelated dead root `logo.png`, rule 7).
- Added `src/lib/runtimeEnv.ts` (`IS_TAURI` detection via `window.__TAURI_INTERNALS__`, `API_BASE` pointing every relative `/api/*` call at `https://clinicmx-web.pages.dev` since the packaged exe has no Functions layer of its own — see the CORS entry below, `openExternal()` for external links/mailto/wa.me) and `src/lib/downloadBlob.ts` (native Save-file dialog instead of the browser's `<a download>` trick, which WebView2 doesn't act on).
- Ported the same fix into every "download/share/external-link" call site: `sharePdf.ts` (all PDF share flows), the 4 CSV exports (`Billing.tsx`, `analytics.ts`, `doctorAnalytics.ts`, `staff.ts`), `deviceBackup.ts`'s local backup download, `whatsappMessages.ts`, the patient-file-open link in `PatientProfile.tsx`, and a capture-phase anchor interceptor in `main.tsx` for the remaining `target="_blank"`/`mailto:` links (Drive link, WhatsApp icon) — Tauri-only, web/APK behavior is unchanged. Skips registering the PWA service worker under Tauri (redundant against an already-local bundle).
- **Any new PDF/file "download" or external link must go through `sharePdf()`/`downloadBlob()`/`openExternal()`, not a raw `<a download>` or `window.open()`** — same rule as the existing Android-APK one below, now with a third platform to break.
- Not yet committed/pushed — local worktree at `D:\Claude\Clinicmx-web-tauri` (branch `tauri-desktop`, based on `main`) pending review.
