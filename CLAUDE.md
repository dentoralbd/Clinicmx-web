...
- **Any new PDF "download" → `sharePdf()`, never `jsPDF.save()`** (2026-08-01, extended 2026-08-09) — the app also ships as a native Android APK and a Windows desktop exe, neither of which acts on a raw `.save()`/`<a download>`. See UI-UX.md §7 for the full pattern; found three PDF generators that skipped it and were broken in the APK. Any *other* file download or external link (CSV export, backup file, `mailto:`/`wa.me`) needs the same treatment on desktop — see "Windows desktop app" below.
...
- **Deciding `isOffline`/`isOfflineFailure` once via `const`, before *multiple* sequential writes, is a bug pattern (2026-08-09).** `handleVisitSubmit` (`PatientProfile.tsx`) decided offline-or-not once, before four writes; a `navigator.onLine`-lying network meant the first write's failure threw straight past three other writes with nothing queued. Any function issuing several sequential Supabase writes offline-capably needs `isOffline` as a `let` that can promote to `true` mid-function on a connectivity failure, with the `groupId` created unconditionally up front so a promoted write still has somewhere to enqueue into — see `handleVisitSubmit`/`handleVisitEditSubmit`/`handleDeleteVisit` for the pattern.
- **`clinicmx_doctor_roster_v1` (2026-08-09, `idb-keyval`, written/read in `PatientProfile.tsx`)** is a device-wide (not per-patient) cache of doctor name → `default_share_pct`, used as a fallback below this patient's own cached treatment history when the `app_users` roster fetch fails or is offline. Any new `app_users` column added to `fetchDoctorsList`'s select that should be usable offline belongs in this cache's payload too, not just `doctorSharePctMap`.
- **`recordInvoicePayment()` (`src/lib/payments.ts`) takes an optional `group: { groupId, seq }` (2026-08-09).** Without it, an offline-queued payment starts its own independent `groupId` — fine when the invoice it's paying already has a real server id (no ordering hazard), but wrong when the invoice itself is *also* still queued (e.g. `InvoiceModal.createInvoiceOffline`): a separately-synced payment group could be manually approved before the invoice group and hit an FK violation against an invoice id the server doesn't have yet. Any new caller building its own multi-step offline group that ends in a payment should pass its own `groupId`/next `seq` through, not accept the default.
- **`src/lib/connectivityStatus.ts` (2026-08-09) feeds the amber offline banner and the Approve & Sync `disabled` gates.** It's driven entirely by `isConnectivityError()` (`supabaseErrors.ts`) calling `markConnectivityFailureObserved()` the moment it matches a real resolved fetch failure — not by `navigator.onLine` or the browser's `online`/`offline` events, which never fire on a degraded-but-interface-up connection (weak signal, packet loss, timeouts). Every existing and future `isOfflineFailure()`/`isConnectivityError()` call site automatically feeds this — no extra wiring needed when adding a new offline-capable write path.

## Windows desktop app (Tauri v2, added 2026-08-09)

`src-tauri/` wraps this same app (WebView2, bundles `dist/`) into a Windows `.exe`/NSIS installer. It is **not** the same thing as `D:\Claude\Clinicmx-web-redesign` — that's a separate Anti-Gravity UI-redesign sandbox that also has its own (older, diverging) Tauri wrapper; this one is the real one, built from this repo's actual code.

- **Commands:** `npm run tauri dev` (loads the Vite dev server in a Tauri window — fast iteration, no Rust rebuild needed for frontend-only changes) / `npm run tauri build` (release exe + NSIS installer under `src-tauri/target/release/`; add `--no-bundle` to skip the installer step and just get `app.exe`). Requires Rust + MSVC C++ Build Tools (rustup.rs) and the WebView2 Runtime (preinstalled on modern Windows).
- **The exe has no server behind it.** Everything that hits a relative `/api/*` path (admin 2FA, Users tab, Drive backup, DentOral bridge) must go through `src/lib/runtimeEnv.ts`'s `API_BASE`, which routes those calls at the live `clinicmx-web.pages.dev` deployment instead. That only works because `functions/api/_middleware.ts` (2026-08-08 entry, CHANGELOG.md) allows CORS from the `tauri.localhost` origin — if that middleware isn't deployed to `main`, admin login in the exe hard-fails with "Login verification is unavailable."
- **Downloads/external links need the Tauri-aware helpers**, not the raw browser APIs — see the `sharePdf()` gotcha above. `src/lib/downloadBlob.ts` opens a native Save dialog (WebView2 doesn't act on `<a download>` + a blob URL); `openExternal()` in `runtimeEnv.ts` opens `mailto:`/`https:`/`wa.me` links via the `opener` plugin instead of navigating the app's own window.
- **Icons** (`src-tauri/icons/`) are generated with `npx tauri icon src-tauri/icon-source.png` — a square, centered, background-matched crop of `public/logo.png` (the real logo isn't square, so feeding it to `tauri icon` directly would stretch/distort the mark). Not the unrelated dead root `logo.png` (rule 7) — that's what the icons were wrongly generated from before this. If the logo changes, remake `icon-source.png` the same way (square canvas, sampled background color, centered) before regenerating.
- **Each rebuild only updates the raw exe/installer under `src-tauri/target/release/`** — it does not touch any previously-installed copy (e.g. a Start Menu shortcut from an earlier install). Reinstall or manually replace the installed `app.exe` after every rebuild, or testing will silently run stale code.
- Debugging a "why did this fail" in the packaged exe with no visible error: `alert()` inside a `catch` is the most reliable way to surface it (no devtools access without extra plugin wiring) — remove before shipping.
