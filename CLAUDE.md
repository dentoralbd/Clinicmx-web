...
- **Four separate places create a patient with an Age-only (no real DOB) fallback:** `Patients.tsx`, `AppointmentModal.tsx`'s inline new-patient, `Consultations.tsx`'s intake form, and `Prescriptions.tsx`'s inline new-patient (`QueueManagement.tsx`'s walk-in quick-add doesn't collect DOB/age at all, so it's not one of them). All four independently derive a synthetic `date_of_birth` (today's month/day, years subtracted — `deriveDateOfBirthFromAge()`, duplicated locally in two of the four rather than shared) and must set `patients.dob_is_estimated = true` when they do, or the Celebrations & Greetings feature (`src/lib/celebrationReminders.ts`, added 2026-08-22) wrongly fires "Birthday Today" on that patient's registration date every year. Found live 2026-08-22 the same day the flag shipped: it was only wired into `Patients.tsx`, and a patient added that day via one of the other three flows still tripped the bug. A new patient-creation entry point needs the same `dobIsEstimated`/`dob_is_estimated` handling as the other four.
